...
namespace Gallatin.Filter.Util
{
    ...
    public static class IpAddressParser
    {
        ...
        /// </summary>
        /// <param name="ipFilter">IP filter (IP address, IP range, IP with mask)</param>
        /// <param name="ipAddress">Target IP address to match</param>
        /// <returns><c>true</c> if the IP address matched the filter</returns>
        ...
        private static bool MaskCheck( string ipFilter, string ipAddress )
        {
            ...
            return RangeCheck( startIP + "-" + endIP, ipAddress );
        }
